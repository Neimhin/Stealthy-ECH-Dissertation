...
\begin{tikzpicture}
    ...
    \def\ivlen{\sechtwoivlen}
    \def\taglen{\sechtwotaglen}
    \def\servernamelen{\sechtwoservernamelen}
    \def\randomlen{\sechtworandomlen}
...
\colorlet{color4}{purple}
    ...
    \pgfmathsetmacro{\ivbegin}{0}
    \pgfmathsetmacro{\ivend}{\ivbegin+\ivlen-1}
    \pgfmathsetmacro{\ciphertextbegin}{\ivend+1}
    \pgfmathsetmacro{\ciphertextend}{\ciphertextbegin+\servernamelen+\randomlen-1}
    \pgfmathsetmacro{\servernamebegin}{\ciphertextbegin}
...
(\ivbegin,0) -- (\ivend*\boxwidth,0) node[midway,yshift=\textshift cm]{AEAD \nonce};

\draw[decorate,decoration={brace,amplitude=10pt,raise=4pt},yshift=\shiftfar cm]
(\tagbegin*\boxwidth,0) -- (\tagend*\boxwidth,0) node[midway,yshift=\textshift cm]{MAC $t$};

\draw[decorate,decoration={brace,amplitude=10pt,raise=4pt},yshift=\shiftfar cm]
(\ciphertextbegin*\boxwidth,0) -- (\ciphertextend*\boxwidth,0) node[midway,yshift=\textshift cm]{encrypted text};
